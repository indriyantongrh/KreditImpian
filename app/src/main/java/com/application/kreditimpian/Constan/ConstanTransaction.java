package com.application.kreditimpian.Constan;

/**
 * Created by indriyanto Nugroho on 30 Jan 2020.
 */
public class ConstanTransaction {

    public static final String KEY_ID_TRANSACTION = "id";
    public static final String KEY_STATUS= "status";
    public static final String KEY_NUMBER = "number";
    public static final String KEY_REFERENCE_ID = "reference_id";
    public static final String KEY_NAME_PRODUCT_TRANSACTION = "name";
    public static final String KEY_FILENAME_TRANSACTION = "filename";
    public static final String KEY_ID_PRODUCT_CATEGORY_TRANSACTION = "id_product_category";
    public static final String KEY_PRICE_CAPITAL_TRANSACTION = "price_capital";
    public static final String KEY_PRICE_SALE_TRANSACTION = "price_sale";
    public static final String KEY_DISCOUNT_TRANSACTION = "discount";
}
